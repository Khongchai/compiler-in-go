...
func TestIntegerArithmetic(t *testing.T) {
	tests := []compilerTestCase{
		...
		// Order of this one will be reversed by the compiler
		{
			input:             "1 < 2",
			expectedConstants: []interface{}{2, 1},
			expectedInstructions: []code.Instructions{
...
// Note, the compiler does not care whether the object to be indexed
// is indexable, that is the job of the vm. Here, we're just testing
// if there exist indeed an OpIndex.
...
`,
			expectedConstants: []interface{}{
				...
				// Because all functions are closures, they must retain access to everything that is neither global
				// nor local. Hence `OpGetFree` even though it is not used by the middle function.
				[]code.Instructions{
					code.Make(code.OpGetFree, 0),
					code.Make(code.OpGetLocal, 0),
